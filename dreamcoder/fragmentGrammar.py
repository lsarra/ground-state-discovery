...
class FragmentGrammar(object):
    def __init__(self, logVariable:float, productions:List[tuple]):
        # (likelihood, type, primitive)
...
"""
    ...
    def __str__(self):
        ...
        return "\n".join([f"{self.logVariable}\tt0\t$_"] + ["%f\t%s\t%s" % (l, t, p)
    ...
    def closedUses(self, request, expression) -> Tuple[float, Uses]:
    ...
    def expectedUses(self, frontiers)-> Uses:
        if len(list(frontiers)) == 0:
            return Uses()
        
        likelihoods = []
        for frontier in frontiers:
            frontier_likelihoods = []
            for entry in frontier:
                for l, u in [self.closedUses(frontier.task.request, entry.program)]:
                    frontier_likelihoods.append((l+entry.logLikelihood, u))
            likelihoods.append(frontier_likelihoods)
        
        # likelihoods = [
        #     [(l + entry.logLikelihood, u) for entry in frontier for l, u in [self.closedUses(frontier.task.request, entry.program)]]
        #                for frontier in frontiers]
    ...
    def insideOutside(self, frontiers, pseudoCounts:float)->"FragmentGrammar":
        """Estimate the grammar parameters using the given frontiers"""
        uses = self.expectedUses(frontiers)
        return FragmentGrammar(logVariable=log(uses.actualVariables + pseudoCounts) - log(max(uses.possibleVariables, 1.)), 
                               productions=[(log(uses.actualUses.get(p, 0.) + pseudoCounts) -log(uses.possibleUses.get(p, 0.) + pseudoCounts), t, p) 
    ...
    def uniform(productions):
        # logVariable, (likelihood, type, primitive)
    ...
    def makeUniform(self)->"FragmentGrammar":
    ...
    def induceFromFrontiers(
        ...
        def grammarScore(g:"FragmentGrammar"):
        ...
        if aic is not POSITIVEINFINITY:
            ...
            while True:
                ...
                restrictedFrontiers = restrictFrontiers()
                # TODO this takes a big part of the compute time
                fragments = []
                proposedFragments = proposeFragmentsFromFrontiers(restrictedFrontiers, arity, CPUs=CPUs)
                for f in proposedFragments:
                ...
                if not candidateGrammars: break

                # chunksize: Each process handles up to 100 grammars at a time, a "job"
                # maxTasks: Maximum number of jobs allocated to a process
                # After evaluating this*chunk many grammars, we killed the process, freeing up its memory.
                # In exchange we pay the cost of spawning a new process.
                # TODO: figure out how big we can make it without running out of memory.
                scoredFragments = parallelMap(CPUs, grammarScore, candidateGrammars,
                                              chunksize=max(1, min(len(candidateGrammars) // CPUs, 100)),
                                              maxtasksperchild=5)
                newScore, newGrammar = max(scoredFragments, key=lambda score_grammar: score_grammar[0])
                ...
                eprint(f"New primitive of type {newType}\t{newPrimitive}\t\n(score = {newScore}; dScore = {dS}; <uses> = {expectedUses})")
                ...
                frontiers = parallelMap(
                    CPUs, lambda frontier: bestGrammar.rescoreFrontier(RewriteFragments.rewriteFrontier(frontier, newPrimitive)), 
                    frontiers)
                eprint(f"\t(<uses> in rewritten frontiers: {bestGrammar.expectedUses(frontiers).actualUses[concretePrimitive]})")
        else:
        ...
        # TODO: Enable again
        eprint(f"Old joint = {oldJoint}\tNew joint = {bestGrammar.jointFrontiersMDL(frontiers, CPUs=CPUs)}\n")
        ...
        for p in bestGrammar.primitives:
            eprint(f"{productionUses[p]} / {possibleUses[p]}\t{p}")

        bestGrammar.clearCache()
        grammar = bestGrammar.toGrammar()

        # Enable to remove unused primitives
